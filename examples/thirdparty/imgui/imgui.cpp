    ...
    : DrawListInst(&context->DrawListSharedData)
{
    ...
    // [NVIDIA modification]
    for (size_t i = 0; i < ImGuiWindowResizeHandle_COUNT; ++i)
        ResizeHandles[i] = true;
}
...
static bool ImGui::UpdateManualResize(ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_held, int resize_grip_count, ImU32 resize_grip_col[4])
{
    ...
    if (!window->ResizeHandles[ImGuiWindowResizeHandle_Corner])
        resize_grip_count = 0;
    ...
    {
        // BEGIN NVIDIA modification
        if (!window->ResizeHandles[border_n])
            continue;
        // END NVIDIA modification
