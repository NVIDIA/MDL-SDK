...
//---------- Output primitives
std::string DFA::Ch(unsigned ch) {
	char format[16];
	if (ch < ' ' || ch >= 127 || ch == '\'' || ch == '\\')
		coco_snprintf(format, sizeof(format), "0x%x", ch);
	else
		coco_snprintf(format, sizeof(format), "'%c'", (char)ch);
	return std::string(format);
}

std::string DFA::ChCond(unsigned ch) {
	return "ch == " + Ch(ch);
}
...
void DFA::PutRange(CharSet *s) {
	for (CharSet::Range *r = s->head; r != NULL; r = r->next) {
		if (r->from == r->to) {
			...
			fprintf(gen, "ch == %s", from.c_str());
		} else if (r->from == 0) {
			std::string to = Ch(r->to);
			fprintf(gen, "ch <= %s", to.c_str());
		} else {
			std::string from = Ch(r->from);
			std::string to = Ch(r->to);
			fprintf(gen, "(ch >= %s && ch <= %s)", from.c_str(), to.c_str());
		}
...
void DFA::DeleteRedundantStates() {
	std::vector<State *> newState(lastStateNr + 1);
	...
	for (State *state = firstState->next; state != NULL; state = state->next) {
		if (used[state->nr]) {
			state->nr = ++lastStateNr; lastState = state;
		} else {
...
void DFA::Step(State *from, Node const *p, BitArray &stepped) {
	if (p == NULL) {
		return;
	}
	...
	case Node::iter:
		...
		Step(from, p->sub, stepped);
		if (p->state != from) {
			BitArray newStepped(tab.nodes.size());
...
void DFA::FindTrans(Node const *p, bool start, BitArray &marked) {
	if (p == NULL || marked[p->n]) {
		return;
	}
	marked.Set(p->n, true);
	if (start) {
		BitArray stepped(tab.nodes.size());
...
void DFA::ConvertToStates(Node *p, Symbol *sym) {
	...
	BitArray marked(tab.nodes.size());
	...
	for (State *state = firstState; state != NULL; state = state->next) {
		...
		for (Action *action = state->firstAction; action != NULL; action = action->next) {
			...
			if (action->typ == Node::clas) {
				fprintf(trace, "%s", tab.classes[action->sym]->name.c_str());
			} else {
				fprintf(trace, "%3s", Ch((char)action->sym).c_str());
			}
			for (Target *targ = action->target; targ != NULL; targ = targ->next) {
				fprintf(trace, "%3d", targ->state->nr);
			}
			if (action->tc == Node::contextTrans) {
				fprintf(trace, " context\n");
			} else {
				fprintf(trace, "\n");
			}
		}
...
Melted* DFA::NewMelted(BitArray *set, State *state) {
	Melted *m = new Melted(set, state, firstMelted);
	return firstMelted = m;
...
Melted* DFA::StateWithSet(BitArray const &s) {
...
std::string DFA::CommentStr(Node *p) {
	std::string s;
	while (p != NULL) {
		if (p->typ == Node::chr) {
			s += (char)p->val;
...
void DFA::GenComBody(Comment *com) {
	...
	std::string res = ChCond(com->stop[0]);
	if (com->stop == "\n") {
		// allow EoF as a synonym for '\n' if it ends a comment
		fprintf(gen, "\t\t\tif (%s || ch == buffer->EoF) ", res.c_str());
	} else {
		fprintf(gen, "\t\t\tif (%s) ", res.c_str());
	}
	fprintf(gen, "{\n");
	...
	if (com->stop.size() == 1) {
		...
		fprintf(gen, "\t\t\t\tNextCh();\n");
		std::string res = ChCond(com->stop[1]);
		fprintf(gen, "\t\t\t\tif (%s) {\n", res.c_str());
		fprintf(gen, "\t\t\t\t\tlevel--;\n");
		fprintf(gen, "\t\t\t\t\tif (level == 0) { oldEols = line - line0; NextCh(); return true; }\n");
		fprintf(gen, "\t\t\t\t\tNextCh();\n");
	...
	if (com->nested) {
			fprintf(gen, "\t\t\t}");
			std::string res = ChCond(com->start[0]);
			fprintf(gen, " else if (%s) ", res.c_str());
			fprintf(gen, "{\n");
		if (com->stop.size() == 1)
			fprintf(gen, "\t\t\t\tlevel++; NextCh();\n");
		else {
			fprintf(gen, "\t\t\t\tNextCh();\n");
			std::string res = ChCond(com->start[1]);
			fprintf(gen, "\t\t\t\tif (%s) ", res.c_str());
			fprintf(gen, "{\n");
			fprintf(gen, "\t\t\t\t\tlevel++; NextCh();\n");
			fprintf(gen, "\t\t\t\t}\n");
...
void DFA::GenLiterals () {
	std::vector<Symbol *> *ts[2] = { &tab.terminals, &tab.pragmas };
...
int DFA::GenNamespaceOpen(std::string const &nsName) {
	if (nsName.empty()) {
		return 0;
	}
...
void DFA::WriteState(State *state) {
	...
	for (Action *action = state->firstAction; action != NULL; action = action->next) {
		...
		if (action->typ == Node::chr) {
			std::string res = ChCond((char)action->sym);
			fprintf(gen, "%s", res.c_str());
		} else {
...
void DFA::WriteScanner() {
	...
	if (firstComment != NULL) {
		...
		while (com != NULL) {
			std::string res = ChCond(com->start[0]);
			fprintf(gen, "(%s && Comment%d())", res.c_str(), cmdIdx);
			if (com->next != NULL) {
				fprintf(gen, " || ");
			}
			com = com->next; cmdIdx++;
