...
void RemoteMemoryManager::notifyObjectLoaded(ExecutionEngine *EE,
  ...
  SmallVector<MISTD::pair<Allocation, uint64_t>, 16> Offsets;
  ...
  for (size_t i = 0, e = NumSections; i != e; ++i) {
    ...
    if (Section.IsCode) {
      ...
      Offsets.push_back(MISTD::pair<Allocation,uint64_t>(Section, CurOffset));
      CurOffset += Size;
    }
  }
...
bool RemoteMemoryManager::finalizeMemory(MISTD::string *ErrMsg) {
  // FIXME: Make this function thread safe.
