...
static int RunProgramRemotelyWithTimeout(StringRef RemoteClientPath,
  ...
  if (255 == ReturnCode) {
    ...
    MISTD::string StdOutFileName = StdOutFile.str();
    MISTD::ifstream ErrorFile(StdOutFileName.c_str());
    if (ErrorFile) {
      MISTD::copy(MISTD::istreambuf_iterator<char>(ErrorFile),
                MISTD::istreambuf_iterator<char>(),
                MISTD::ostreambuf_iterator<char>(OS));
      ErrorFile.close();
    }
...
static MISTD::string ProcessFailure(StringRef ProgPath, const char** Args,
                                  unsigned Timeout = 0,
                                  unsigned MemoryLimit = 0) {
  MISTD::ostringstream OS;
  OS << "\nError running tool:\n ";
  ...
  MISTD::ifstream ErrorFile(ErrorFilename.c_str());
  if (ErrorFile) {
    MISTD::copy(MISTD::istreambuf_iterator<char>(ErrorFile),
              MISTD::istreambuf_iterator<char>(),
              MISTD::ostreambuf_iterator<char>(OS));
...
namespace {
  class LLI : public AbstractInterpreter {
    MISTD::string LLIPath;          // The path to the LLI executable
    MISTD::vector<MISTD::string> ToolArgs; // Args to pass to LLI
  public:
    LLI(const MISTD::string &Path, const MISTD::vector<MISTD::string> *Args)
      : LLIPath(Path) {
      ToolArgs.clear ();
...
int LLI::ExecuteProgram(const MISTD::string &Bitcode,
                        const MISTD::vector<MISTD::string> &Args,
                        const MISTD::string &InputFile,
                        const MISTD::string &OutputFile,
                        MISTD::string *Error,
  ...
  MISTD::vector<const char*> LLIArgs;
  LLIArgs.push_back(LLIPath.c_str());
  LLIArgs.push_back("-force-interpreter=true");

  for (MISTD::vector<MISTD::string>::const_iterator i = SharedLibs.begin(),
...
static MISTD::string PrependMainExecutablePath(const MISTD::string &ExeName,
                                             const char *Argv0,
                                             void *MainAddr) {
  // Check the directory that the calling program is in.  We can do
  // this if ProgramPath contains at least one / character, indicating that it
  // is a relative path to the executable itself.
  MISTD::string Main = sys::fs::getMainExecutable(Argv0, MainAddr);
...
AbstractInterpreter *AbstractInterpreter::createLLI(const char *Argv0,
                                     ...
                                     const MISTD::vector<MISTD::string> *ToolArgs) {
  MISTD::string LLIPath =
...
namespace {
  class CustomCompiler : public AbstractInterpreter {
    MISTD::string CompilerCommand;
    MISTD::vector<MISTD::string> CompilerArgs;
  public:
    CustomCompiler(
      const MISTD::string &CompilerCmd, MISTD::vector<MISTD::string> CompArgs) :
      CompilerCommand(CompilerCmd), CompilerArgs(CompArgs) {}

    virtual void compileProgram(const MISTD::string &Bitcode,
                                MISTD::string *Error,
                                unsigned Timeout = 0,
                                unsigned MemoryLimit = 0);
...
void CustomCompiler::compileProgram(const MISTD::string &Bitcode,
                                    MISTD::string *Error,
                                    unsigned Timeout,
                                    unsigned MemoryLimit) {

  MISTD::vector<const char*> ProgramArgs;
  ProgramArgs.push_back(CompilerCommand.c_str());

  for (MISTD::size_t i = 0; i < CompilerArgs.size(); ++i)
...
namespace {
  class CustomExecutor : public AbstractInterpreter {
    MISTD::string ExecutionCommand;
    MISTD::vector<MISTD::string> ExecutorArgs;
  public:
    CustomExecutor(
      const MISTD::string &ExecutionCmd, MISTD::vector<MISTD::string> ExecArgs) :
...
int CustomExecutor::ExecuteProgram(const MISTD::string &Bitcode,
                        const MISTD::vector<MISTD::string> &Args,
                        const MISTD::string &InputFile,
                        const MISTD::string &OutputFile,
                        MISTD::string *Error,
  ...
  for (MISTD::size_t i = 0; i < ExecutorArgs.size(); ++i)
...
static void lexCommand(MISTD::string &Message, const MISTD::string &CommandLine,
                       MISTD::string &CmdPath, MISTD::vector<MISTD::string> Args) {

  MISTD::string Command = "";
  MISTD::string delimiters = " ";

  MISTD::string::size_type lastPos = CommandLine.find_first_not_of(delimiters, 0);
  MISTD::string::size_type pos = CommandLine.find_first_of(delimiters, lastPos);

  while (MISTD::string::npos != pos || MISTD::string::npos != lastPos) {
    MISTD::string token = CommandLine.substr(lastPos, pos - lastPos);
    if (Command == "")
       Command = token;
    else
  ...
  if (CmdPath.empty()) {
    Message =
      MISTD::string("Cannot find '") + Command +
      "' in PATH!\n";
    return;
  }
...
//
GCC::FileType LLC::OutputCode(const MISTD::string &Bitcode,
                              MISTD::string &OutputAsmFile, MISTD::string &Error,
  ...
  MISTD::vector<const char *> LLCArgs;
...
void LLC::compileProgram(const MISTD::string &Bitcode, MISTD::string *Error,
                         unsigned Timeout, unsigned MemoryLimit) {
  MISTD::string OutputAsmFile;
  OutputCode(Bitcode, OutputAsmFile, *Error, Timeout, MemoryLimit);
  sys::fs::remove(OutputAsmFile);
}

int LLC::ExecuteProgram(const MISTD::string &Bitcode,
                        const MISTD::vector<MISTD::string> &Args,
                        const MISTD::string &InputFile,
                        const MISTD::string &OutputFile,
                        MISTD::string *Error,
  ...
  MISTD::vector<MISTD::string> GCCArgs(ArgsForGCC);
...
namespace {
  class JIT : public AbstractInterpreter {
    ...
    MISTD::vector<MISTD::string> ToolArgs; // Args to pass to LLI
  public:
    JIT(const MISTD::string &Path, const MISTD::vector<MISTD::string> *Args)
      : LLIPath(Path) {
      ToolArgs.clear ();
      if (Args) { ToolArgs = *Args; }
    }
...
int JIT::ExecuteProgram(const MISTD::string &Bitcode,
                        const MISTD::vector<MISTD::string> &Args,
                        const MISTD::string &InputFile,
                        const MISTD::string &OutputFile,
                        MISTD::string *Error,
  ...
  MISTD::vector<const char*> JITArgs;
...
AbstractInterpreter *AbstractInterpreter::createJIT(const char *Argv0,
                   MISTD::string &Message, const MISTD::vector<MISTD::string> *Args) {
  MISTD::string LLIPath =
...
static bool IsARMArchitecture(MISTD::vector<const char*> Args) {
  for (MISTD::vector<const char*>::const_iterator
...
int GCC::ExecuteProgram(const MISTD::string &ProgramFile,
                        const MISTD::vector<MISTD::string> &Args,
                        FileType fileType,
                        const MISTD::string &InputFile,
                        const MISTD::string &OutputFile,
                        ...
                        unsigned Timeout,
                        unsigned MemoryLimit) {
  MISTD::vector<const char*> GCCArgs;
  ...
  for (MISTD::vector<MISTD::string>::const_iterator
...
int GCC::MakeSharedObject(const MISTD::string &InputFile, FileType fileType,
...
///
GCC *GCC::create(MISTD::string &Message,
                 const MISTD::string &GCCBinary,
                 const MISTD::vector<MISTD::string> *Args) {
  MISTD::string GCCPath = sys::FindProgramByName(GCCBinary);
  if (GCCPath.empty()) {
    Message = "Cannot find `"+ GCCBinary +"' in PATH!\n";
    return 0;
  }

  MISTD::string RemoteClientPath;
