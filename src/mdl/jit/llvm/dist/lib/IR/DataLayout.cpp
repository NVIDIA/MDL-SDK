...
public:
  ...
  void erase(StructType *STy) {
      LayoutInfoTy::iterator I = LayoutInfo.find(STy);
      if (I != LayoutInfo.end()) {
          StructLayout *Value = I->second;
          LayoutInfo.erase(I);
          free(Value);
...
bool DataLayout::doFinalization(Module &M) {
  sys::SmartScopedLock<true> Scope(LayoutLock);
  delete static_cast<StructLayoutMap*>(LayoutMap);
  LayoutMap = 0;
  return false;
}
...
const StructLayout *DataLayout::getStructLayout(StructType *Ty) const {
  StructLayout *L = NULL;
  {
    ...
    L = (StructLayout *)malloc(sizeof(StructLayout)+(NumElts-1) * sizeof(uint64_t));
...
void DataLayout::deleteLayoutCache() const
{
  sys::SmartScopedLock<true> Scope(LayoutLock);
  StructLayoutMap *STM = static_cast<StructLayoutMap*>(LayoutMap);
  if (STM)
