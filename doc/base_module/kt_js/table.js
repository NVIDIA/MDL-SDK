...
function adjust_slanted_header_table() {
	...
	$(table).find(slant_select).each( function() {
	    max_slant_height = Math.max
	    (max_slant_height, 
	     $(this).outerWidth() * Math.sin(angle) - 
	     $(this).closest("td").outerHeight());
	});
	var translate = "translate3d(0px, " + max_slant_height + "px, 0px)";
	var container = $(table).closest(".tablecontainer");
	if ($(container).length == 1) {
	    $(container).css("transform", translate);
	    $(container).css("margin-bottom", max_slant_height + "px");
	} else {
	    $(table).css("transform", translate);
	    $(table).css("margin-bottom", (max_slant_height + 10 ) + "px");
	}
    });
}
